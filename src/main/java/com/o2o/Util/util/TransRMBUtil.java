...
public class TransRMBUtil {
	//定义常量
	private static final  float NUMBER_05F=0.5f;
	 ...
	 * @return
	 */
	public String transRMB(String money) {
		checkNum(money);
		StringBuffer buffer = new StringBuffer();
		buffer.append(this.cleanZero(this.splitNum(this.roundString(money))));
	...
	private String roundString(String s) {
		...
		d = (d * NumberConstants.NUMBER_100 + NUMBER_05F) / NumberConstants.NUMBER_100;
		...
		if (intOnly.length() > NumberConstants.NUMBER_13) {
	...
	private String numFormat(int flag, String s) {
		...
		for (int i = 0; i < sLength; i++) {
			...
			if (flag == 1) {
				...
				newS = newS + bigLetter[s.charAt(i) - NUMBER_48] + unit[sLength - i - 1];

			} else if (flag == 2) {

				// 转换小数部分（带单位）

				newS = newS + bigLetter[s.charAt(i) - NUMBER_48] + small[sLength - i - 1];
	...
	private String cleanZero(String s) {
		...
		for (int i = 0; i < NumberConstants.NUMBER_3; i++) {
